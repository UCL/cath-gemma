...
sub load_scans_data_of_starting_clusters_and_dir {
	state $check = compile( Invocant, Path, CathGemmaCompassProfileType, ArrayRef[Str] );
	my ( $proto, $scans_dir, $compass_profile_build_type, $starting_clusters ) = $check->( @ARG );
	...
	foreach my $initial_scan ( @$initial_scans ) {
		...
		my $filename = scan_filename_of_dir_and_cluster_ids( $scans_dir, [ $query_id ], $match_cluster_ids, $compass_profile_build_type );
...
sub load_scans_data_of_gemma_dir_set {
	state $check = compile( Invocant, CathGemmaDiskGemmaDirSet, CathGemmaCompassProfileType, Optional[ArrayRef[Str]] );
	my ( $proto, $gemma_dir_set, $compass_profile_build_type, $starting_clusters ) = $check->( @ARG );

	$starting_clusters //= $gemma_dir_set->get_starting_clusters();

	return __PACKAGE__->load_scans_data_of_starting_clusters_and_dir( $gemma_dir_set->scan_dir(), $compass_profile_build_type, $starting_clusters );
}
...
sub load_scans_data_of_dir_and_starting_clusters_dir {
	state $check = compile( Invocant, Path, CathGemmaCompassProfileType, Path );
	my ( $proto, $scan_dir, $compass_profile_build_type, $starting_clusters_dir ) = $check->( @ARG );
...
sub load_scans_data_of_base_dir_and_project {
	state $check = compile( Invocant, Path, Str, CathGemmaCompassProfileType );
	my ( $proto, $base_dir, $project, $compass_profile_build_type ) = $check->( @ARG );
