...
subtest 'constructs_from_no_arguments' => sub {
	new_ok( 'Cath::Gemma::Scan::Impl::Links' );
};

subtest 'builds_correctly_from_score_of_id_of_id' => sub {
	my $scans_data = Cath::Gemma::Scan::Impl::Links->new_from_score_of_id_of_id( make_example_score_of_id_of_id() );
	isa_ok( $scans_data, 'Cath::Gemma::Scan::Impl::Links' );
};

subtest 'builds_correctly_from_score_of_id_of_id_with_few_scores' => sub {
	my $scans_data = Cath::Gemma::Scan::Impl::Links->new_from_score_of_id_of_id( { item_1 => {}, item_2 => { item_1 => 5.0 } } );
	isa_ok( $scans_data, 'Cath::Gemma::Scan::Impl::Links' );
...
subtest 'add_separate_starting_clusters_works' => sub {
	...
	is     ( $scans_data->add_separate_starting_clusters( [ qw/ new_thing_1 new_thing_2 / ] ), $scans_data, 'add_separate_starting_clusters() returns the Links object' );
...
subtest 'add_scan_entry_works' => sub {
	...
	is( $scans_data->get_score_between( 'item_1', 'otherx'       ), undef,       'Request for absent score returns undef'          );
	is( $scans_data->add_scan_entry   ( 'item_1', 'otherx', 20.0 ), $scans_data, 'add_scan_entry() returns the Links object'       );
...
subtest 'removes_correctly' => sub {
	...
	is( $scans_data->remove( 'item_2' ), $scans_data, 'remove() returns the Links object' );
	is_deeply(
		$scans_data->sorted_ids(),
