...
**Input** : &nbsp; A bunch of non-overlapping "starting clusters" of sequences. Each starting cluster has an identifying integer, `N` and an associated file `N.faa` containing the sequences in the cluster.

**Output** : &nbsp; A `.trace` file specifying an ordered list of pairwise cluster-merge operations that, together, merge the starting clusters into one root node. Each line of the file is of the format:
<br>
...
**Scores** : &nbsp; The evalues between clusters are generated by COMPASS comparisons between alignments of the sequences in those clusters. When two clusters are merged, a combined alignment is generated for the new cluster and is COMPASS-scored against the other clusters' alignments.
...
1. **Existing Code** The current code produces similar but slightly different results. This may well be due to changed versions of MAFFT/CD-HIT. A brief investigation suggested that trying to chase this up would be more effort than it'd be worth.
...
I propose:
 ...
 1. Build a well-designed, well-tested, well-commented, well-documented script <small>(and associated libraries)</small> that reproduces small/medium trees in standalone (ie single-machine, non-HPC) mode
 1. Extend this to:
...
After `A` and `B` are merged, we'd want `C` to be merged with `A+B` before `D` <small>*(assuming the evalue between `C` and `A+B` is comparably small)*</small>, like this:
 ...
 * do all-versus-all comparisons between the members of any sub-cluster pairs that for a cross S3 cluster link
...
Some issues 10th March 2017 for meeting
--
